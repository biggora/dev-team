> **NOTE (2026-07)** — this historical walkthrough predates the evidence-gate, adversarial-planning, and review-contract revisions. It does not describe the mandatory PRD/plan creator → `adversarial-reviewer` debate, cycle-3 arbitration, or the later ordinary document review. The `brainstorming`/`writing-plans`/`using-superpowers` skills mentioned below were removed, the report protocol now requires an `Evidence` field plus `Context` (every dispatch's `Required reading` source accounted for) and `Self-check`/`Sweep` (per the `review-contract` skill), the planner produces vertical slices, and the tester runs in Mode A (red) / Mode B (green). The sample report blocks below also predate the `RV-<scope>-NNN` finding schema (classes `must-fix-now`/`fix-in-slice`/`backlog`, the late-finding rule, and the disposition protocol) that now governs ordinary review and rework. For current behavior see `specs/workflow.md` and `CLAUDE.md`.
