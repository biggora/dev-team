...
- **Review-and-rework pattern**: only open `must-fix-now` findings trigger a rework dispatch — the creator returns exactly one disposition per RV-ID (`accepted_and_fixed`, `rejected_with_evidence`, `needs_decision`); re-dispatch limit of 3 per scope+role with attempt context passed to the agent
