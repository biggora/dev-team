...
- **Task table**: slice/subtask, assigned agent, status, one-line Evidence summary, open `RV-` IDs with class, and the attempt count for that scope+role
- **Technical debt**: `### Technical debt` in `docs/progress.md` (not the task table) lists every `backlog`-classed finding — RV-ID, artifact, and reason. Under Micro, where no ledger exists, backlog findings go into the coordinator's final report instead.
...
**Ordinary review contract**: findings follow the `review-contract` skill — stable `RV-<scope>-NNN` IDs with a class, exactly one of `must-fix-now` (blocks the gate), `fix-in-slice` (real but non-blocking; fixed by that agent's next scheduled dispatch in the same slice, never a dedicated rework round), or `backlog` (debt, recorded, never blocks). Only open `must-fix-now` findings block a gate and consume the rework budget: at most 2 rework dispatches per artifact per gate, counted on open `must-fix-now` findings rather than on dispatches; `fix-in-slice` and `backlog` findings never consume this budget. **Late-finding rule**: on a recheck the reviewer carries every prior RV-ID forward and may raise a new `must-fix-now` only when the rework itself introduced it (state the rationale) or it is a Critical correctness or security defect — anything else noticed for the first time is `backlog`; a reviewer that violates this is re-dispatched once with the rule quoted, and its late findings do not extend the budget. **Rework packet**: re-dispatch the creator with the complete finding list and require exactly one disposition per ID — `accepted_and_fixed`, `rejected_with_evidence` (cite it), or `needs_decision` — the same discipline the debate gate already applies to `CH-*` IDs. If the same failure signature appears 3 times, change strategy ONCE or escalate with the full attempt history. Never loop. `adversarial-reviewer` keeps `CH-*` IDs and its own independent budget below, separate from `RV-*` — unchanged.
...
Every debate dispatch is self-contained. Include the original request verbatim; artifact type, path, and version; initial pass or current cycle and maximum; complete mode-specific challenge ledger; latest dispositions and cited evidence; verdict; unresolved IDs; related artifact paths and decisions; scope boundaries; Required reading (the artifact under debate plus any document it must stay consistent with — same rule as Phase 2 step 1); stack/version context; output/report format; and the evidence reminder.
...
6. Then **per slice**, in order (this per-slice protocol applies to ANY plan with slices — greenfield or feature work on an existing project):
   ...
   **DoD gate: do not start slice N+1 until slice N's acceptance tests pass end-to-end, code review is DONE, the demo checkpoint happened, and the slice's evidence was produced against the running local stack (or `Local stack: N/A` is recorded). Deviation only by explicit user decision recorded in `docs/progress.md` with a debt-closure slice; an open deviation past its deadline blocks all further slices. Every open `fix-in-slice` finding from slice N's reviews is fixed or explicitly re-classed `backlog` with a recorded reason before slice N+1 starts.**
...
1. For each subtask, construct a complete agent prompt that includes:
   ...
   - **Required reading**: one line per source, `<path> → <what to extract>`. Name every normative document that exists and constrains this scope — `docs/prd.md`, `docs/use-cases.md`, `docs/architecture.md`, `docs/design.md`, `docs/plan.md` — plus the relevant `docs/progress.md` decisions and the Phase 0 prior-art precedent file when one exists. Never write "read the documentation": agents do not read documents you did not name, and a path without an extraction instruction is read superficially. The agent accounts for every listed source in its `Context:` report field (see the `review-contract` skill). This obligation governs every dispatch prompt this skill constructs — creator, reviewer, debate, and Phase 4 alike; each site below adds only its minimum. Every entry must be readable with the target agent's tool grant — never name a shell command for a Read/Grep/Glob-only agent (code-reviewer, doc-reviewer, adversarial-reviewer); paste the command's output into the prompt instead.
   - **Review state**: for every review or recheck dispatch (ordinary or debate), the coordinator — not the reviewer — owns the scope tag and the next free number. Include the artifact's scope tag (`SLICE<n>`, `PRD`, `PLAN`, `ARCH`, `INFRA`, or `MICRO` under Micro), the next free `RV-<scope>-NNN`, and the full prior RV-ID list with each ID's state — or state this is the first review of the artifact.
...
- **After architect**: If the architecture is routine (existing project, well-known patterns, fewer than 5 components), accept without separate doc-reviewer dispatch — downstream agents will surface issues; record "Architecture review: skipped — routine". If novel (greenfield, unfamiliar patterns, 5+ components), dispatch doc-reviewer: "Review docs/architecture.md for consistency with docs/prd.md, clear component responsibilities, explicit interfaces, and implementation sequence." Maximum 1 rework (not 2). Then pass "Read docs/architecture.md" to planner and implementation agents. Required reading for that dispatch: `docs/architecture.md` → the blueprint under review; `docs/prd.md` → the consistency baseline.
- **After ui-ux-designer**: If the design is routine (existing project, minor UI addition, fewer than 3 screens), accept without separate doc-reviewer dispatch; record "Design review: skipped — routine". If substantial (greenfield, 3+ screens, new design system), dispatch doc-reviewer: "Review docs/design.md for consistency with docs/prd.md, hex color palette, wireframes, component states, role coverage of the use cases, responsive behavior, and accessibility." Maximum 1 rework (not 2). Then pass "Read docs/design.md" to frontend-dev and tester. Required reading for that dispatch: `docs/design.md` → the spec under review; `docs/prd.md` → the consistency baseline; `docs/use-cases.md` → role coverage, when it exists.
...
- **After implementor / devops-engineer / backend-dev / frontend-dev / tester**: dispatch code-reviewer per the `review-contract` skill's rubric for that agent's changes (test code additionally checked for weakened assertions, skip/only, deleted tests, and assert-nothing tests; devops-engineer's changes additionally checked for pinned image tags, a health check per service, named volumes removed by `down -v`, deterministic host ports, no production credentials, and `.env.example` completeness), including stack-specific version context. On concerns, re-dispatch the original agent with the rework packet (see Review and Debate Limits). Then: after devops-engineer, pass service names, host ports, and env-var names to backend-dev, frontend-dev, and tester in every later dispatch; after backend-dev, pass API endpoints and response formats to frontend-dev (if dispatched sequentially). Required reading for that dispatch: the changed files listed in scope; `docs/architecture.md` and `docs/plan.md` when they exist and bear on the change.
...
3. For each report, take action based on status:
   ...
   | BLOCKED | Read the blocker. Provide the missing resource/information and re-dispatch, or ask the user for help. A reviewer BLOCKED on scope size is answered by dispatching the split it proposed, not by re-dispatching the same scope — split dispatches do not count against the rework budget |
...
3b. **Context gate**: a report whose `Context:` field does not account for every source listed in that dispatch's Required reading is treated as DONE_WITH_CONCERNS — re-dispatch naming the sources that were not accounted for. This costs one cheap re-dispatch and prevents the shallow-work rework cycle it would otherwise cause. A source the agent's tool grant cannot reach is accounted for as `<path> → unreachable with this agent's tools` and does not fail the gate.
...
2. **Cross-cutting code review** (if multiple code agents were dispatched):
   ...
   - **Required reading**: the complete changed-file list above; `docs/architecture.md` and `docs/plan.md` when they exist
...
3. **Cross-document review** (if docs/ files were created or modified):
   ...
   - **Required reading**: every `docs/*.md` file present, listed by path
