...
2. **Dispatch creator** using the Agent tool:
   ...
   - **Required reading**: pass a `Required reading` block naming `docs/prd.md`, `docs/architecture.md`, and `docs/design.md` where they exist, one line each as `<path> → <what to extract>`; planner accounts for every line in its `Context:` report field (`review-contract` skill).
...
5. **Ordinary review or arbitration** — a separate protocol and budget from the `CH-*` debate above; see the `review-contract` skill:
   - On challenger `CONSENSUS` with no unresolved IDs, dispatch `dev-team:doc-reviewer` for a full plan review; findings carry `RV-PLAN-NNN` IDs with class `must-fix-now`, `fix-in-slice`, or `backlog`. Only open `must-fix-now` findings block and consume the 2-round rework budget; re-dispatch planner with the complete finding list, requiring exactly one disposition per ID (`accepted_and_fixed`, `rejected_with_evidence` with citation, or `needs_decision`), then re-dispatch doc-reviewer to recheck — the late-finding rule applies on recheck.
   - After an unresolved third recheck, the challenger returns `ARBITRATION_REQUIRED`; dispatch doc-reviewer with the complete plan and ledger to arbitrate all items and perform the full review together, using the same `RV-PLAN-NNN` schema for any residual findings. A successful result needs no additional ordinary review.
